...
public class ExponentialBackOff {
    ...
    protected ExponentialBackOff(Builder builder) {
        ...
        if (!(initialIntervalMillis > 0)) throw new IllegalArgumentException();
        if (!(0 <= randomizationFactor && randomizationFactor < 1)) throw new IllegalArgumentException();
        if (!(multiplier >= 1)) throw new IllegalArgumentException();
        if (!(maxIntervalMillis >= initialIntervalMillis)) throw new IllegalArgumentException();
        if (!(maxElapsedTimeMillis > 0)) throw new IllegalArgumentException();
        reset();
    }
